...
try:
    ...
    class _TreeFallback:
        @staticmethod
        def map_structure(fn, value):
            if isinstance(value, BatchFeature):
                return BatchFeature(data={k: _TreeFallback.map_structure(fn, v) for k, v in value.items()})
            if isinstance(value, dict):
                return {k: _TreeFallback.map_structure(fn, v) for k, v in value.items()}
            if isinstance(value, tuple):
                return tuple(_TreeFallback.map_structure(fn, v) for v in value)
            if isinstance(value, list):
                return [_TreeFallback.map_structure(fn, v) for v in value]
            return fn(value)
...
class GR00T_Tokenizer(PreTrainedModel):
    ...
    def __init__(
        ...
        dinov2_path_override: str | None = None,
    ):
        ...
        if dinov2_path_override is not None:
            config.backbone_cfg['dinov2_path'] = str(dinov2_path_override)
        ...
        dinov2_path = config.backbone_cfg.get('dinov2_path', None)
        if not dinov2_path or not Path(str(dinov2_path)).exists():
            raise FileNotFoundError(
                "UniT tokenizer requires a local DINOv2 checkpoint path. "
                "Set framework.latent_action.dinov2_path_override to a local directory. "
        ...
        from .tokenizer.vision_branch_encoder import (
        ...
        from .tokenizer.action_branch_encoder import (
        ...
        from .tokenizer.visual_action_fusion import QFormerVisualActionFusion
        ...
        from .tokenizer.vector_quantizer import (
        ...
        from .tokenizer.latent_motion_decoder import LatentMotionDecoder
        ...
        from .tokenizer.action_branch_decoder import (
    ...
    def from_pretrained(cls, pretrained_model_name_or_path: str, **kwargs):
        """Load a tokenizer checkpoint from local files only."""
        local_model_path = Path(str(pretrained_model_name_or_path))
        if not local_model_path.exists():
            raise FileNotFoundError(
                f"UniT tokenizer checkpoint must be a local path, got: {pretrained_model_name_or_path}"
            )

        model_config = AutoConfig.from_pretrained(local_model_path.as_posix(), local_files_only=True)
        ...
        print(f"Loading pretrained tokenizer from local path: {local_model_path}")
        print(f"Tune vision model (shared): {tune_vision_model}")
        print(f"Tune vision M-Former: {tune_vision_m_former}")
        print(f"Tune bridge projector: {tune_bridge_projector}")
        print(f"Tune action encoder: {tune_action_encoder}")
        ...
        try:
            pretrained_model = super(GR00T_Tokenizer, cls).from_pretrained(
                ...
                config=model_config,
                local_model_path=local_model_path.as_posix(),
