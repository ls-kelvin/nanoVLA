...
"""Flow-matching noise schedule used by the Wan2.1 video branch.

Ported from harobotsDL ``robotsdl/models/policy/expert/wan/scheduler.py``.
Kept as a standalone copy so starVLA does not import that repo.
"""
...
class FlowMatchScheduler:
    ...
    def training_weight(self, timestep):
        timestep_id = torch.argmin(
            (self.timesteps[:, None].to(timestep.device) - timestep[None]).abs(),
            dim=0,
        )
