"""QwenPI_v3 with bridge-token latent-action supervision."""
...
class Qwen_PI_v3_LA(Qwen_PI_v3):
    ...
    def _collect_latent_window_counts(self, examples: List[dict]) -> list[int]:
        ...
        for example in examples:
            ...
            if frames is None:
                raise ValueError(
                    "QwenPI_v3_LA requires `la_frames`. Use datasets.vla_data.dataset_py=lerobot_la_datasets "
                    "and enable datasets.vla_data.latent_action."
                )
            if self.latent_action_backend == "villax":
                if not isinstance(frames, list) or len(frames) == 0 or not isinstance(frames[0], list):
                    raise ValueError(
                        "QwenPI_v3_LA (villax) requires `la_frames` to be a list of clips. "
        ...
        if len(set(counts)) != 1:
            raise ValueError(
                "QwenPI_v3_LA requires the same latent-action window count per batch "
    ...
    def _build_latent_frame_pairs(self, examples: List[dict]):
        ...
        for example in examples:
            ...
            if frames is None or len(frames) < 2:
                raise ValueError(
                    "QwenPI_v3_LA requires `la_frames` with at least two frames for latent-action targets."
                )
            counts.append(len(frames) - 1)
    ...
    def _encode_vl_hidden_states_with_latent_tokens(
        ...
        with torch.autocast("cuda", dtype=torch.bfloat16):
            ...
            if self.num_action_dit_layers > 0:
                vl_embs_list = [
                    hidden[:, :prompt_len, :]
    ...
    def _make_villax_targets(self, examples: List[dict]) -> torch.LongTensor:
        ...
        for example in examples:
            ...
            if clips is None or not isinstance(clips, list) or len(clips) == 0 or not isinstance(clips[0], list):
                raise ValueError(
                    "QwenPI_v3_LA (villax) requires `la_frames` as a list of clips. "
                    "Set datasets.vla_data.latent_action.full_window=true."
                )
    ...
    def _print_training_sample_once(self, examples: List[dict], instructions: list[str]) -> None:
        ...
        example = examples[0]
        logger.info(
            "First QwenPI_v3_LA training sample (episode_index=%s, step_index=%s, episode_path=%s):\n"
            "  instruction: %s\n"
            "  la_frame_offsets: %s\n"
            "  num_bridge_tokens: %s\n"
            "  latent backend: %s",
    ...
    def forward(self, examples: List[dict] = None, **kwargs) -> dict:
        ...
        if compute_action:
            ...
            if action_examples:
                action_dit_loss = self._continuous_action_loss(
                    action_examples,
                    action_vl_embs_list,
