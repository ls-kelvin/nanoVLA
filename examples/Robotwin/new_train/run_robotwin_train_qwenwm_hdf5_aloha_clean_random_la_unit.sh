...
###########################################################################################
# QwenWM_LA + UniT on RoboTwin2.0 raw HDF5 aloha-agilex EEF data.
# Latent-action learning is inside the action head (joint flow matching over
# [latent, state, action]); latent targets are UniT pre-quantization continuous
# embeddings. Main/action dataloader: joint forward. Latent dataloader: latent only.
###########################################################################################
...
config_yaml=./examples/Robotwin/new_train/starvla_qwenwm_hdf5_aloha_clean_random_la_unit.yaml
run_root_dir=./results/Checkpoints2
data_mix=hdf5_aloha_clean_eef
latent_data_mix=hdf5_aloha_clean_random_eef
run_id=0704_${data_mix}_joint_action_latent_qwenwm_la_unit
batch_size=8
hdf5_root=/inspire/qb-ilm/project/qproject-fundationmodel/public/zzt/data/RoboTwin2.0/dataset
unit_tokenizer_ckpt=/inspire/qb-ilm/project/qproject-fundationmodel/public/zzt/starVLA/third_party/UniT/outputs/robotwin_hdf5_multiview_endpose_tokenizer_robomind_replace_aloha_random
