...
config_yaml=./examples/Robotwin/new_train/starvla_qwenwmv34_hdf5_aloha_clean_random_la_sharla_foresight_soft_kl.yaml
...
run_id=0823_${data_mix}_action_${latent_data_mix}_latent_qwenwmv34_la_sharla_foresight_soft_kl
...
# 缓存必须先跑一遍，且路径/stride 只能改 YAML（该脚本除 --overwrite 外不接受其他参数）：
# bash scripts/cache_sharla_soft_kl_episodes.sh "${config_yaml}"
